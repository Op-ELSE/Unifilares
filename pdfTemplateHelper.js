 ...
 * @param {Object} data  // keys: systemVoltage, upstreamBreaker, shortCircuit, … etc.
...
async function populateTemplate(data, eppImageBase64) {
  ...
  // -------------------------------------------------
  // Page 1 – Text fields (coordinates you provided)
  // -------------------------------------------------
  const page1 = doc.getPage(0);

  // Insert optional EPP image
  if (eppImageBase64) {
  ...
  for (const [key, value] of Object.entries(textMap)) {
    ...
    if (['Text1','Text2','Text3','Text4','Text5'].includes(key)) {
      ...
      color = PDFLib.rgb(0.2, 0.2, 0.2);          // gris oscuro
    } else if (key === 'Text8') {
    ...
    const drawOptions = {
      x: fx,
      y: fy - 12,      // pequeño ajuste vertical
      size: 12,
      color,
      ...(font && { font })
    };
  ...
  // -------------------------------------------------
  // Page 2 – Text fields (coordinates you provided)
  // -------------------------------------------------
  ...
  // Si deseas que el PDF sea *no editable* descomenta la siguiente línea:
  // pdfDoc.flatten();
